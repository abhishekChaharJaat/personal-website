...
const Projects = () => {
  const projects = [
    {
      name: "Ecommerce Website (ShopShare)",
      description:
    ...
    >
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {projects.map((project, index) => (
          ...
          >
            <h4 className="text-[#18475A] font-medium text-[14px] md:text-[16px] mb-1 flex items-center justify-between">
              {project.name}
              <a
                href={project.projectUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-[#13866F] rounded-full px-3 py-1 text-[11px] font-semibold shadow-sm border border-transparent hover:border-[#0f5a4c] hover:shadow-md transition duration-200"
              >
                View Work <FaExternalLinkAlt size={10} />
              </a>
            </h4>
            <p className="text-[#1B2B48] text-[14px] md:text-[16px] mb-3">
              {project.description}
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech, i) => (
                <span
                  key={i}
                  className="bg-[#E0F2F1] text-[#13866F] px-2 py-[2px] text-[11px] rounded-full font-medium"
                >
                  {tech}
                </span>
              ))}
